...
function run(cmd, opts = {}) {
  console.log(`\n$ ${cmd}`)
  execSync(cmd, { stdio: 'inherit', cwd: opts.cwd || rootDir, env: { ...process.env, ...(opts.env || {}) }, shell: true })
}
...
try {
  ...
  if (fs.existsSync(path.join(serverDir, 'package.json'))) {
    console.log('\n📦 安装 server 依赖...')
    run('pnpm install --omit=dev --no-audit', { cwd: serverDir })
  }

  // 2. 构建 uni-app H5
  if (fs.existsSync(path.join(rootDir, 'vite.config.ts'))) {
    console.log('\n📦 构建 H5 前端 (uni build:h5)...')
    run('pnpm run build:h5', { env: { NODE_ENV: 'production' } })

    // 3. 将 H5 构建产物拷贝到 server/public
