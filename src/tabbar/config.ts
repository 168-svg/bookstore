...
const _tabbar: TabBar = {
  ...
  iconWidth: '24px',
  spacing: '3px',
  // isBulge 的项只是操作型按钮，不是真正的 tabBar 页面，不能塞进 list 里，
  // 否则小程序会把它当成 tabBar 页，从而导致 navigateTo 返回 error code -3
  list: _tabbarList.filter((item: any) => !item.isBulge) as unknown as TabBar['list'],
}
